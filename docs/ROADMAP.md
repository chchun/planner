...
## Phase 5 — Vercel 배포 (Neon + 서버리스 + Blob)
...
- 백엔드: Hono 앱을 `server/app.ts`로 추출 → 로컬 node-server / Vercel `api/[[...route]].ts` 두 진입점 공유
- **메모 이미지: Vercel Blob** — DB엔 URL만 저장(base64 dataURL 폐기), bootstrap 경량화. 오프라인 표시는 SW 캐싱
- 서버리스 보정: Google push를 `waitUntil`로(응답 후 유실 방지), 부팅 재시도를 Vercel Cron으로, 프로덕션 Secure 쿠키
